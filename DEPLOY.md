...
- iPhone 可用 SVG 图标；**Android 通常需要 PNG**，项目已包含 `pwa-192.png`、`pwa-512.png`。
- 若之前安装过旧版，请：**删除桌面快捷方式** → Chrome 重新打开网址 → 再「添加到主屏幕」。
- 仍无图标：Chrome → 设置 → 隐私 → 清除 **logan-yk.github.io** 的站点数据后重试（会清空该站账本，请先确认无重要未备份数据）。
