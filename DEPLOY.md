...
正确地址：**https://logan-yk.github.io/Shanxinjilu/**

#### 方案 A：Vivo 自带浏览器（你当前的方式）

可以用，账本同样存在**本机浏览器**里。但要知道：

- Vivo 浏览器加桌面一般是**网页快捷方式**，不是完整 PWA；图标支持较弱，可能仍是空白或默认地球图标（**不是 bug，是浏览器限制**）。
- 清除「缓存/历史」**不等于**清除账本；要清 **Cookie / 网站数据**（见下文）。
- 清数据、装图标都必须在 **Vivo 浏览器**里做；清 Chrome **不会**删掉 Vivo 浏览器里的账。

**添加到桌面（Vivo 浏览器）：**

1. Vivo 浏览器打开 **https://logan-yk.github.io/Shanxinjilu/**
2. 底部或右上角 **菜单（≡ 或 ⋮）** → **收藏** / **添加到书签**（先收藏一次有助于识别标题）
3. 再进菜单 → **添加到桌面** / **添加快捷方式到桌面**
4. 以后**只从桌面图标进**，不要混用 Chrome 或微信打开（否则是另一份空账本）

**彻底清空 Vivo 浏览器里的账本：**

1. 长按桌面「善心记录」→ **移除**
2. 打开 **Vivo 浏览器** → **设置**（通常在「我的」或菜单里）
3. **隐私与安全** / **清除数据** → 时间选 **全部**
4. 务必勾选 **Cookie**、**网站数据**（或「账号与网站数据」），不要只勾「缓存」
5. 若仍清不掉：**设置** → **应用** → **应用管理** → **浏览器**（Vivo 浏览器）→ **存储** → **清除数据**（会清空该浏览器所有网站数据）
6. 重新打开网址，确认金额为 0 后，再 **添加到桌面**

**若桌面图标仍空白：** 可忽略，或改用下方 Chrome；**不影响记账功能**。

#### 方案 B：Google Chrome（推荐，图标最稳）

1. 安装 **Chrome**，用 Chrome 打开同一网址
2. 菜单 → **添加到主屏幕** / **安装应用**
3. 账本与 Vivo 浏览器**不互通**；在 Chrome 记账需从零开始（或继续只用 Vivo 浏览器）
